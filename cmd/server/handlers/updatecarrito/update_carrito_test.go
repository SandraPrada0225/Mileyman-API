...
func TestOkUpdateCarrito(t *testing.T) {
	...
	movementsResult := responses.MovementsResult{
...
func TestWhenNotFoundShouldReturn404(t *testing.T) {
	...
	mockUpdateCarrito.On("Execute", mockCarritoID2, getMockMovements()).Return(responses.MovementsResult{}, database.NewNotFoundError(""))
...
func TestWhenInternalServerErrorShouldReturn500(t *testing.T) {
	...
	mockUpdateCarrito.On("Execute", mockCarritoID, getMockMovements()).Return(responses.MovementsResult{}, database.NewInternalServerError(""))
	request := httptest.NewRequest("PUT", "/api/carritos/1", &body)
	request.Header.Add("Content-type", "application/json")
	response := httptest.NewRecorder()
